<script lang="ts">
	...
	const visibleNightlySlots = $derived(slotsForRelease(nightly, DOWNLOAD_SLOTS));
	...
	onMount(() => {
		...
		const slots = slotsForRelease(stable, DOWNLOAD_SLOTS);
		if (platform !== 'unknown') {
...
<svelte:head>
	<title>{$t('ren_browser.downloads.title')} | Ren Browser</title>
...
<div class="max-w-5xl mx-auto space-y-10 pb-8">
	...
	>
		<p class="text-xs text-zinc-500">{$t('ren_browser.downloads.privacy_note')}</p>
		...
		{/if}
		<p class="text-xs text-zinc-500">{$t('ren_browser.downloads.footer_note')}</p>
		<a
			href="/ren-browser"
