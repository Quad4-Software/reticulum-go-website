...
Python 1.3.2 tears down links at LINKIDENTIFY when the remote identity is blackholed. Reticulum-Go does the same via pkg/blackhole during link identify. Federation publish (publish_blackhole /list) and remote sources (blackhole_sources) are implemented. See [Compatibility](/docs/compatibility).
...
| Property tests    | pkg/buffer/_\_pbt_test.go, pkg/resource/__pbt_test.go |
